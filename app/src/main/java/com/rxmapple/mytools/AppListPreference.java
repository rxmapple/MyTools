...
/**
 * Created by xiaoming.ren on 2016/9/28.
 */
public class AppListPreference extends ListPreference {
    ...
    public void setApplications(CharSequence[] apps, CharSequence SelectApp) {
        // Look up all package names in PackageManager. Skip ones we can't find.
        PackageManager pm = getContext().getPackageManager();
        final int entryCount = apps.length + (mShowItemNone ? 1 : 0);
        List<CharSequence> applicationNames = new ArrayList<>(entryCount);
        List<CharSequence> validatedAppStrings = new ArrayList<>(entryCount);
        List<Drawable> entryDrawables = new ArrayList<>(entryCount);
        int selectedIndex = -1;
        for (int i = 0; i < apps.length; i++) {
            try {
                String appComponetStr = apps[i].toString();
                ComponentName cn = ComponentName.unflattenFromString( appComponetStr );
                ApplicationInfo appInfo = pm.getApplicationInfo(cn.getPackageName(), 0);
                ...
                validatedAppStrings.add(appComponetStr);
                entryDrawables.add(appInfo.loadIcon(pm));

                if (appComponetStr.equals( SelectApp )) {
                    selectedIndex = i;
                }
        ...
        if (mShowItemNone) {
            ...
            validatedAppStrings.add(ITEM_NONE_VALUE);
        ...
        setEntryValues(
                validatedAppStrings.toArray(new CharSequence[validatedAppStrings.size()]));
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        if (state instanceof SavedState) {
            ...
            setApplications(savedState.entryValues, savedState.value);
